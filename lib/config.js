...
var config = {
  init: function() {
    argv = yargs
          .usage('Usage: $0 --port [number] --base [string] --context [string] --fallback [string] --enable-query --ignore-queries v token --content-type "application/json; charset=utf-8" --proxy-host https://www.google.com')
          .alias('p', 'port').string('p').default('p', '4080').describe('p', 'listening port')
          .alias('o', 'origin').string('o').default('o', '*').describe('o', 'used as Access-Control-Allow-Origin, if can not inferred from header')
          .alias('a', 'allow-headers').string('a').describe('a', 'extra headers in Access-Control-Allow-Headers')
          ...
          .alias('x', 'proxy-host').string('x').describe('x', 'miss matched request will be sent to proxy host')
          .alias('d', 'debug').describe('d', 'debug mode')
