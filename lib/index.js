...
app.use(function *(next) {
  ...
  this.response.set('Access-Control-Allow-Headers', 'Accept, Authorization, Content-Type');
  yield next;
});
...
app.use(function *() {
  ...
  console.log([chalk[colorMap[this.response.status] || 'yellow'](this.response.status), (t2 - t1) + 'ms', this.request.method, this.request.href, filePath].join(' - '));
});
