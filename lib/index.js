...
var hostMap;
try {
  hostMap = require(path.join(baseDir, '.hostmap.json'));
} catch (err) {
  hostMap = null;
}
var app = koa();
...
app.use(function* (next) {
  var urlObj, origin;
  var allowHeaders = ['Accept', 'Authorization', 'Content-Type', 'Accept-Language', 'X-Requested-With'];
  if (config.allowHeaders) {
    allowHeaders = allowHeaders.concat(config.allowHeaders.split(/\s*,\s*/));
  }
  ...
  origin == '*' || this.response.set('Access-Control-Allow-Credentials', 'true');
  this.response.set('Access-Control-Allow-Methods', 'GET, HEAD, PUT, PATCH, POST, DELETE');
  this.response.set('Access-Control-Allow-Headers', allowHeaders.join(', '));
  yield next;
});
